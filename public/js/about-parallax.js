(function () {
    ...
    if (!body || !body.classList.contains('about-parallax')) return;

    const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (prefersReducedMotion) return;

    const hero = document.querySelector('[data-about-hero]');
    const heroContent = hero ? hero.querySelector('.about-hero-content') : null;
    const parallaxItems = Array.from(document.querySelectorAll('[data-about-parallax]'));
    ...
    function updateHero(multiplier) {
        ...
        hero.style.setProperty('--about-hero-bg-y', bgOffset.toFixed(2) + 'px');

        if (heroContent) {
            heroContent.style.setProperty('--about-hero-content-y', (-latestScrollY * 0.07 * multiplier).toFixed(2) + 'px');
            heroContent.style.setProperty('--about-hero-opacity', String(clamp(1 - progress * 0.38, 0.62, 1)));
        }
    }
    ...
    function updateParallaxItems(multiplier) {
        ...
        parallaxItems.forEach(function (item) {
            ...
            const speed = Number(item.dataset.aboutSpeed || 24);
            const offset = progress * speed * multiplier;

            item.style.setProperty('--about-parallax-y', offset.toFixed(2) + 'px');
        });
    }
    ...
    function initReveal() {
        revealItems.forEach(function (item) {
            const delay = Number(item.dataset.aboutRevealDelay || 0);
            item.style.setProperty('--about-reveal-delay', delay + 'ms');
        });
